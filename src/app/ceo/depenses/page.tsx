...
export default function CEOExpensesPage() {
  ...
  const loadExpenses = async () => {
    setLoading(true);
    try {
      const response = await fetchWithSession(`/api/ceo/depenses?period=${period}`, undefined, {
        loginPath: '/login/ceo',
      });
      if (!response.ok) throw new Error('Impossible de charger les dépenses.');
      const json = (await response.json()) as CEOExpensesData;
      setData(json);
    } catch {
      setData(null);
    } finally {
    ...
    <div className="min-h-screen bg-slate-50 p-8 text-slate-900">
      <div className="mx-auto max-w-7xl">
        ...
        <section className="rounded-2xl border bg-white p-6 shadow-sm">
          ...
          <p className="mt-2 text-sm text-slate-500">Liste des dépenses sur la période sélectionnée.</p>
              ...
              <table className="min-w-full text-left text-sm">
                ...
                <tbody>
                  {data?.expenses?.length ? (
                    ...
                    <tr>
                      <td colSpan={4} className="py-12 text-center text-slate-500">Aucune dépense enregistrée pour cette période.</td>
                    </tr>
                  )}
                </tbody>
              </table>
